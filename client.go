	...
	for {
		...
		if errRead == nil {
			if c.handler != nil {
				...
				if errHandler != nil {
					Error(fmt.Sprintf("Client.handler error ClientID:%v err:%v", c.conn.ConnIndex, errHandler))
				}
			}
		} else {
			//discard current request
			Error(fmt.Sprintf("Client.handleConn error ClientID:%v err:%v", c.conn.ConnIndex, errRead))
			break
		}
	}
}
